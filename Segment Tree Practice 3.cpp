...
vector <int> seg, lazy, vec_input, vec1;
...
int32_t main()
{
    ...
    seg.resize(4 * n + 2);
    lazy.resize(4 * n + 2);
    vec_input.resize(n + 1);
    vec1.resize(n + 1);
    for (int i = 1; i <= n; i++)
    {
        cin >> vec_input[i];
        vec1[i] = vec1[i - 1] + vec_input[i];
    }
    build(1, n, 1);
    char c;
    for (int i = 1; i <= q; i++)
    {
        ...
        if (c == 'P')
        {
            if (a == 1) cout << query(1, n, a, b, 1) << "\n";
            else cout << query(1, n, a, b, 1) - query(1, n, a - 1, a - 1, 1) << "\n";
        }
        else
        {
            update(1, n, a, n, b - vec_input[a], 1);
            vec_input[a] = b;
        }
    }
}
